<script lang="ts">
	...
	import type { HotProduct } from '$lib/adapters/admin';
	...
	function startCrawl() {
		const items: HotProduct[] = data.items;
		if (items.length === 0) return;

		simItems = items.map((item: HotProduct) => ({
			item,
	...
	function applyMarketFilter(market: string) {
		...
		goto(`/dashboard/crawl/hot-products-sku?${params.toString()}`);
	}
	...
	function crawlSingle(item: HotProduct) {
		if (isRunning) return;
	...
	function retrySingle(item: HotProduct) {
	...
	let previewTotalPages = $derived(Math.ceil((data.items as any[]).length / PREVIEW_PER_PAGE));
	let previewItems = $derived(
		(data.items as any[]).slice((previewPage - 1) * PREVIEW_PER_PAGE, previewPage * PREVIEW_PER_PAGE)
	);
...
<div class="space-y-6" in:fade={{ duration: 200 }}>
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			<h2 class="text-2xl font-bold tracking-tight text-slate-900">핫프로덕트 크롤</h2>
			<p class="mt-1 text-sm text-slate-500">
				공식 API로 가져온 핫프로덕트 각 아이템을 크롤 &middot;
		...
		<div class="flex gap-2">
			{#if !isRunning && simItems.length === 0}
				<button
					...
					class="inline-flex items-center gap-2 rounded-lg bg-orange-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-orange-700 active:scale-[0.97] disabled:opacity-40"
				>
			...
			{:else if isRunning && isPaused}
				<button
					onclick={resumeCrawl}
					class="inline-flex items-center gap-2 rounded-lg bg-orange-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-orange-700"
				>
					<svg class="size-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 010 1.972l-11.54 6.347a1.125 1.125 0 01-1.667-.986V5.653z" />
					</svg>
			...
			{:else}
				...
				<button
					onclick={startCrawl}
					class="inline-flex items-center gap-2 rounded-lg bg-orange-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-orange-700"
				>
					<svg class="size-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
	...
	{#if simItems.length > 0}
		...
		<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
			<div class="px-6 py-5">
				<div class="flex items-center justify-between mb-4">
					<div class="flex items-center gap-3">
						{#if isRunning && !isPaused}
							...
							<span class="text-base font-semibold text-orange-600">크롤 진행중</span>
						{:else if isPaused}
				...
				<div class="h-4 w-full overflow-hidden rounded-full bg-slate-100">
					<div class="flex h-full transition-all duration-300">
						...
						<div class="bg-orange-500 animate-pulse transition-all duration-300" style="width: {totalCount > 0 ? (runningCount / totalCount) * 100 : 0}%"></div>
					</div>
				</div>
			...
			<div class="grid grid-cols-5 gap-px border-t border-slate-200 bg-slate-200">
				...
				<div class="bg-white px-4 py-4 text-center">
					<p class="text-xs font-medium text-orange-500 mb-1">실행중</p>
					<p class="text-2xl font-bold tabular-nums text-orange-600">{runningCount}</p>
				</div>
				<div class="bg-white px-4 py-4 text-center">
				...
				<table class="min-w-full divide-y divide-slate-200">
					<thead class="bg-slate-50 sticky top-0 z-10">
						<tr>
							...
							<th class="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-400 w-28 hidden sm:table-cell">현재가</th>
					...
					<tbody class="divide-y divide-slate-100">
						{#each simItems as sim, idx (sim.item.id)}
							...
							<tr class="transition-colors {sim.status === 'running' ? 'bg-orange-50/60' : sim.status === 'failed' ? 'bg-rose-50/40' : 'hover:bg-slate-50/50'}">
								...
								<td class="px-5 py-2.5 text-center">
									...
									{:else if sim.status === 'running'}
										<svg class="mx-auto size-5 animate-spin text-orange-500" fill="none" viewBox="0 0 24 24">
								...
								<td class="px-5 py-2.5 text-right text-sm font-semibold tabular-nums text-slate-700 hidden sm:table-cell">
									{formatPrice(sim.item.currentPrice, sim.item.currency)}
								</td>
			...
			<table class="min-w-full divide-y divide-slate-200">
				...
				<tbody class="divide-y divide-slate-100">
					{#each previewItems as item, idx (item.id)}
						...
						<tr class="transition-colors {sc?.status === 'running' ? 'bg-orange-50/40' : sc?.status === 'failed' ? 'bg-rose-50/30' : sc?.status === 'success' ? 'bg-emerald-50/30' : 'hover:bg-slate-50/60'}">
							<td class="px-5 py-3 text-sm tabular-nums text-slate-400">{(previewPage - 1) * PREVIEW_PER_PAGE + idx + 1}</td>
							<td class="px-5 py-3">
								<a href={item.productUrl} target="_blank" rel="noopener noreferrer" class="group/link flex items-center gap-1 truncate text-sm font-medium text-slate-800 max-w-[280px] hover:text-blue-600 hover:underline">{item.productName}<svg class="size-3 shrink-0 text-slate-400 group-hover/link:text-blue-500" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg></a>
							...
							<td class="px-5 py-3 text-right text-sm font-semibold tabular-nums text-slate-700 hidden sm:table-cell">
							...
							<td class="px-5 py-3 text-center">
								...
								{:else}
									<button
										...
										class="inline-flex items-center gap-1 rounded-md bg-orange-500/10 px-2 py-1 text-xs font-semibold text-orange-600 transition-colors hover:bg-orange-500/20"
									>
					...
					{#if data.items.length === 0}
						<tr>
							<td colspan="5" class="px-6 py-16 text-center text-sm text-slate-400">크롤 대상 핫프로덕트가 없습니다</td>
						</tr>
					{/if}
				</tbody>
			</table>
			...
			{#if previewTotalPages > 1}
				<div class="flex flex-col items-center gap-2 border-t border-slate-200 px-4 py-3">
						...
						{#each getPageNumbers(previewPage, previewTotalPages) as p}
							...
							{:else}
								<button
									...
									class="size-10 rounded-lg text-sm font-semibold transition-colors {p === previewPage
										? 'bg-slate-900 text-white shadow-sm'
					...
					<p class="text-xs tabular-nums text-slate-500">
						<span class="font-medium text-slate-700">{(previewPage - 1) * PREVIEW_PER_PAGE + 1}–{Math.min(previewPage * PREVIEW_PER_PAGE, data.items.length)}</span>
						/ {data.items.length}건
					</p>
				</div>
			{/if}
	...
	<div class="rounded-xl border border-amber-100 bg-amber-50/50 p-5">
		<div class="flex gap-3">
			...
			<div class="text-sm text-amber-700">
				<p class="font-semibold">핫프로덕트 크롤 안내</p>
				<ul class="mt-2 list-inside list-disc space-y-1 text-xs text-amber-600">
					<li>공식 API로 수집된 핫프로덕트 각 아이템의 상세 페이지를 크롤합니다.</li>
					<li>분당 <strong>100개</strong> rate limit 동일 적용</li>
