<script lang="ts">
	...
	let marketFilter = $derived(data.filters.market);
	...
	let activeCount = $derived(data.hotProducts.data.filter((p) => p.active).length);
...
<div class="space-y-6" in:fade={{ duration: 200 }}>
	...
	<div class="flex gap-1.5">
		{#each [{ value: 'all', label: '전체' }, { value: 'coupang', label: 'Coupang' }, { value: 'aliexpress', label: 'AliExpress' }, { value: 'amazon', label: 'Amazon' }] as f (f.value)}
			<button
				onclick={() => applyMarketFilter(f.value)}
				class="rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors {marketFilter ===
				f.value
			...
			<table class="min-w-full divide-y divide-slate-200">
				...
				<tbody class="divide-y divide-slate-100">
					{#each data.hotProducts.data as product, index (product.id)}
						{@const market = marketConfig[product.market] ?? {
						...
						<tr class="transition-colors hover:bg-slate-50/50 {product.active ? '' : 'opacity-50'}">
							<td class="px-5 py-3.5 text-xs text-slate-400 tabular-nums">
							...
							<td class="px-5 py-3.5">
								<span
									class="inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[11px] font-semibold {market.color}"
								>
									<span class="size-1.5 rounded-full {market.dot}"></span>
									{market.label}
								</span>
							</td>
							<td class="px-5 py-3.5 text-right text-sm font-semibold text-slate-800 tabular-nums">
								{formatPrice(product.currentPrice, product.currency)}
							</td>
							<td class="px-5 py-3.5 text-center">
								<span
									class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-semibold {crawl.color}"
								>
									<svg
										class="size-3"
										fill="none"
							...
							<td class="px-5 py-3.5 text-center">
								...
								>
									...
									<button
										...
										aria-label="{product.productName} {product.active
											? '미노출로 변경'
											: '노출로 변경'}"
									>
										{#if product.active}
		...
		{#if data.hotProducts.totalPages > 1}
			<div class="flex flex-col items-center gap-2 border-t border-slate-200 px-4 py-3">
				<div class="flex items-center gap-1">
					...
					{#each getPageNumbers(data.hotProducts.page, data.hotProducts.totalPages) as p, i (i)}
						{#if p === '...'}
				...
				<p class="text-xs text-slate-500 tabular-nums">
					<span class="font-medium text-slate-700"
						>{(data.hotProducts.page - 1) * data.hotProducts.limit + 1}–{Math.min(
							data.hotProducts.page * data.hotProducts.limit,
							data.hotProducts.total
						)}</span
					>
