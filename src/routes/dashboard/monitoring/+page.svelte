<script lang="ts">
	...
	let marketFilter = $state(data.filters.market);
	...
	function applyFilters() {
		...
		params.set('page', '1');
		goto(resolve(`/dashboard/monitoring?${params.toString()}`));
	}
				...
				<div class="flex items-center justify-between">
					...
					<span class="flex items-center gap-1.5">
						<span class="relative flex h-2.5 w-2.5">
							...
							{/if}
							<span
								class="relative inline-flex h-2.5 w-2.5 rounded-full {statusColorMap[
									market.status
								]}"
							></span>
						</span>
		...
		<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
			...
			<div class="flex items-center gap-3">
				...
				<div class="flex gap-1">
					{#each [{ value: 'all', label: '전체' }, { value: 'success', label: '성공' }, { value: 'failed', label: '실패만' }] as f (f.value)}
						<button
							onclick={() => {
								statusFilter = f.value;
								applyFilters();
							}}
							class="rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors {statusFilter ===
							f.value
								? f.value === 'failed'
			...
			<div class="overflow-x-auto">
				<table class="min-w-full divide-y divide-slate-200">
					...
					<tbody class="divide-y divide-slate-200 bg-white">
						{#each data.crawlLogs.data as log (log.id)}
							<tr class="transition-colors hover:bg-slate-50/50">
								<td class="px-6 py-3 text-xs whitespace-nowrap text-slate-500">
								...
								<td class="max-w-[200px] px-6 py-3 text-sm">
									<a
										...
										class="group/link flex items-center gap-1 truncate text-slate-700 hover:text-blue-600 hover:underline"
									>
										{log.productName}
										<svg
			...
			{#if data.crawlLogs.totalPages > 1}
				<div class="flex flex-col items-center gap-2 border-t border-slate-200 px-4 py-3">
					<div class="flex items-center gap-1">
						...
						{#each getPageNumbers(data.crawlLogs.page, data.crawlLogs.totalPages) as p, i (i)}
							{#if p === '...'}
								<span class="flex size-10 items-center justify-center text-sm text-slate-400"
									>…</span
								>
							{:else}
								<button
					...
					<p class="text-xs text-slate-500 tabular-nums">
						<span class="font-medium text-slate-700"
							>{(data.crawlLogs.page - 1) * data.crawlLogs.limit + 1}–{Math.min(
