<script lang="ts">
	...
	const menuSections = [
		...
		},
		{
			title: '운영',
			items: [
				{ name: '배치 관리', href: '/dashboard/batch', icon: 'Cube' },
				{ name: '토큰 관리', href: '/dashboard/tokens', icon: 'Key' }
			]
		},
		{
			title: '모니터링',
...
>
	...
	>
		...
		>
			{#if $sidebar.isOpen}
				<span class="text-lg font-bold tracking-tight whitespace-nowrap text-blue-400"
					>Price Eye</span
				>
			{:else}
	...
	<nav class="flex flex-1 flex-col space-y-6 overflow-x-hidden overflow-y-auto px-3 py-4">
		{#each menuSections as section (section.title)}
			<div class="space-y-1">
				{#if $sidebar.isOpen}
					<h3
						class="mb-2 px-2 text-xs font-semibold tracking-wider whitespace-nowrap text-slate-400 uppercase"
					>
						{section.title}
					</h3>
				{:else}
	...
	<div class="border-t border-slate-800 bg-slate-950/30 p-4">
		<div class="flex items-center {$sidebar.isOpen ? '' : 'justify-center'}">
			...
			<div
				class="ml-3 overflow-hidden transition-all duration-300 {$sidebar.isOpen
					? 'w-auto opacity-100'
